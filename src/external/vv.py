...
async def __get_text_query(speaker_id: int, text: str) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(VV_ENDPOINT_QUERY, params=REQUEST_PARAMS) as resp:
...
async def get_speaking_wave(speaker_id: int, text: str) -> bytes:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            VV_ENDPOINT_AUDIO, params=REQUEST_PARAMS, json=REQUEST_JSON
        ) as resp:
